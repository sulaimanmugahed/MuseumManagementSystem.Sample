...
{
    public class ArtifactConditionsRepository : GenericRepository<ArtifactCondition>, IArtifactConditionsRepository
    {
        private readonly ApplicationDbContext _context;
        public ArtifactConditionsRepository(ApplicationDbContext context)
            : base(context)
        {
            _context = context;
        }
        ...
        {
            return _context.Artifacts.Where(a=> a.ArtifactConditionId.ToString() == id).Count();
        }


        public bool IsNameAssigned(string name)
        {
            return _context.ArtifactConditions.Any(s => s.Name == name);
        }

        public bool IsNameAssigned(string name, Guid id)
        {
            return _context.ArtifactConditions.Any(s => s.Name == name && s.Id != id);
        }


        public bool IsContainArtifacts(Guid id)
        {
            return _context.Artifacts.Any(a => a.ArtifactConditionId == id);
        }

        public async Task<PagedResult<ArtifactCondition>> GetAllArtifactConditionsPagedAsync(string? searchValue, int? start, int? length, string? sortColumn = null, string? sortDirection = "ASC")
        {

            var query = _context.ArtifactConditions.AsQueryable();
            ...
            return new PagedResult<ArtifactCondition>
            {
