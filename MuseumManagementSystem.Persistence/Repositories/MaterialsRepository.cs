...
{
    public class MaterialsRepository(ApplicationDbContext context) :
        GenericRepository<Material>(context),
        IMaterialsRepository
    {
        ...
        {
            return _context.Materials.Any(s => s.Name == name);
        }


        public bool IsNameAssigned(string name, Guid id)
        {
            return _context.Materials.Any(s => s.Name == name && s.Id != id);
        }



        public async Task<PagedResult<Material>> GetAllMaterialsPagedAsync(string? searchValue, int? start, int? length, string? sortColumn = null, string? sortDirection = "ASC")
        {
