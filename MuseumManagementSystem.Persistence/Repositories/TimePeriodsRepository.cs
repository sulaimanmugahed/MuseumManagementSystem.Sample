...
{
    public class TimePeriodsRepository(ApplicationDbContext context) :
        GenericRepository<TimePeriod>(context),
        ITimePeriodsRepository
    {
        ...
        {
            return _context.Artifacts.Where(a => a.TimePeriodId.ToString() == id).Count();
        }


        public bool IsNameAssigned(string name, Guid id)
        {
            return _context.TimePeriods.Any(s => s.Name == name && s.Id != id);
        }


        public bool IsNameAssigned(string name)
        {
            return _context.TimePeriods.Any(s => s.Name == name);
        }

        public bool IsContainArtifacts(Guid id)
        {
            return _context.Artifacts.Any(a => a.TimePeriodId == id);
        }

        public async Task<PagedResult<TimePeriod>> GetAllTimePeriodsPagedAsync(string? searchValue, int? start, int? length, string? sortColumn = null, string? sortDirection = "ASC")
        {

            var query = _context.TimePeriods.AsQueryable();
            ...
            return new PagedResult<TimePeriod>
            {
